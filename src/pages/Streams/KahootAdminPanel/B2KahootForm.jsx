...
export const B2KahootForm = ({ destination }) => {
  ...
  const linksSchema = yup.object().shape({
    b2_1: yup.string().optional(),
    b2_2: yup.string().optional(),
    b2_3: yup.string().optional(),
    b2_4: yup.string().optional(),
    b2_5: yup.string().optional(),
  ...
  const handleLinksSubmit = async (values, { resetForm }) => {
    ...
    if (!emptyValues || confirmation) {
      const b2links = { b2: { links: {} } };
      for (const [key, value] of Object.entries(values)) {
        if (value && key !== 'replace') {
          b2links.b2.links[key] = value;
        } else {
          b2links.b2.replace = value;
        }
      }
      try {
